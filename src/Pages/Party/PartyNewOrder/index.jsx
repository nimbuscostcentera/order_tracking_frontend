...
import usePlacePartyOrder from "../../../store/usePartyOrderAdd";
function RegularOrder() {
  ...
  const { PartyList, fetchPartyData } = useFetchParty();
  const {
    PlacePartyOrder,
    PartyOrderSuccess,
    PartyOrderError,
  ...
  useEffect(() => {
    if (isPartyOrderLoading) {
      toast.play("pleaes wait...", {
        position: "top-right",
        autoClose: 3000,
      });
    }
    if (PartyOrderSuccess) {
